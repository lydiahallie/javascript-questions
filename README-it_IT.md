...
`eval` esegue il codice di input come stringa. Se è un'espressione, come in questo caso, viene valutata. L'espressione `10 * 10 + 5` ritorna il numero `105`.
...
###### 22. Per quant'è disponibile `cool_secret`?

```javascript
sessionStorage.setItem("cool_secret", 123);
```

- A: Per sempre, il dato non verrà perso.
- B: Quando l'utente chiude il tab.
- C: Quando l'utente chiudo il browser, non solo il tab.
- D: Quando l'utente spegne il computer.
...
Il dato salvato nello `sessionStorage` è rimosso dopo la chiusura del _tab_.

Se usassi `localStorage`, il dato sarebbe persistito per sempre, a meno che `localStorage.clear()` sia richiamato.
...
Con la keyword `var`, puoi dichiarare multiple variabili con lo stesso nome. La variabile conterrà solo l'ultimo valore.

Questo non lo puoi fare con `let` o `const` perché esistono solo all'interno dello scope.
...
Tutte le chiavi degli oggetti (ad esclusione dei `Symbols`) sono stringe, anche se non vengono digitate come stringhe. Per questo `obj.hasOwnProperty('1')` risponde con `true`.

Questo non funziona per i `Set`. Non c'è `'1'` nel nostro set: `set.has('1')` returns `false`. Il set come chiave contiene valore numerico `1`, `set.has(1)` returns `true`.
...
Se si hanno due chiavi dello stesso nome, la chiave sarà sostituita. Sarà sempre alla stessa possizione, ma con l'ultimo valore specificato.
...
###### 26. Il contesto globale di JavaScript crea due cose: l'oggetto `global`, e la parola chiave "this".
...
Il contesto di esecuzione globale è accessibile ovunque nel tuo codice sorgente.
...
Lo statement `continue` salta un iterazione se una certa condizione è `true`.
...
`String` è un costruttore pre costruito a cui possiamo aggiungere proprietà. Ho solo aggiunto una proprietà al suo `prototype`. Le stringhe sono automaticamente convertite in oggetti stringa, generati dal `prototype`. Per questo tutte le stringe (string objects) hanno accesso a quel modello!
