...
1. Foo.getName();
输出：alert(4);
这里直接调用了 Foo 函数对象的 getName 方法，因此会弹出 4。

2. getName();
输出：alert(2);
在 var getName = function() {...} 这行代码之后，getName 指向了一个匿名函数，因此执行时会弹出 2。

3. Foo().getName();
输出：alert(5);
这里先调用 Foo()，由于 Foo() 函数内部重新定义了全局的 getName 变量为一个新的函数，所以 Foo() 的返回值是 this，即全局对象 window（如果在浏览器环境下执行）。
接着，对返回的 window 执行 getName()，由于全局的 getName 已经被重新定义，所以弹出 5。

4. getName();
输出：alert(5);
由于在第3步中执行了 Foo()，重定义了全局的 getName 为一个新函数，所以此时再调用 getName()，会弹出 5。

5. new Foo.getName();
输出：alert(4);
这里使用 new 关键字调用了 Foo.getName 函数，而 Foo.getName 是一个普通函数，并不是构造函数，因此 new 关键字对它没有实际效果，直接执行 Foo.getName()，弹出 4。

6. new Foo().getName();
输出: alert(3);
这里 new Foo() 创建了一个新对象，并调用了 Foo 构造函数，然后调用这个新对象上的 getName 方法。因为 Foo.prototype.getName 定义了一个函数，所以输出 alert(3)。

7. new new Foo().getName();
输出: alert(3);
这里 new Foo() 创建了一个新对象，并调用了 Foo 构造函数，然后再次使用 new 关键字调用该新对象上的 getName 方法。因为 Foo.prototype.getName 定义了一个函数，所以输出 alert(3)。
...
Foo().getName() 弹出 5
getName() 弹出 5
new Foo.getName() 弹出 4
new Foo().getName() 弹出 3
new Foo().getName() 弹出 3
注意：代码中的 Foo() 函数定义和 var getName 的重定义都会对全局作用域产生影响，可能会导致一些不可预测的行为，尤其是在浏览器环境下。为了避免出现意外情况，应尽量避免在全局作用域下声明和修改变量，最好将它们限制在函数作用域中。
