...
<details><summary><strong> در این مرجع یا رفرنس ۲۰ ترجمه موجود است : 🇸🇦🇪🇬🇧🇦🇩🇪🇪🇸🇫🇷🇮🇩🇯🇵🇰🇷🇳🇱🇧🇷🇷🇺🇹🇭🇹🇷🇺🇦🇻🇳🇨🇳🇹🇼🇽🇰</strong></summary>
<p>
...
---
‌####### سوالات آمده در این ریپو یا مخزن از نوع ۴ گزینه ای هستند.
###### ۱. خروجی آن چیست؟ 
...
function sayHi() {
  console.log(name);
  console.log(age);
  var name = "Lydia"; // Lydia یک اسم لاتین است
  let age = 21;
}
...
- ۱: `Lydia` and `undefined`
- ۲: `Lydia` and `ReferenceError`
- ۳: `ReferenceError` and `21`
- ۴: `undefined` and `ReferenceError`
...
در داخل تابع، ابتدا متغیر "name" را با کلمه کلیدی "var" اعلام می کنیم. این بدان معناست که متغیر بالا می‌رود (فضای حافظه در مرحله ایجاد تنظیم می‌شود) با مقدار پیش‌فرض «تعریف نشده»، تا زمانی که در واقع به خطی که در آن متغیر را تعریف می‌کنیم برسیم. ما هنوز متغیر را در خطی که می‌خواهیم متغیر «name» را ثبت کنیم، تعریف نکرده‌ایم، بنابراین همچنان مقدار «undefined» را حفظ می‌کند.

متغیرهای دارای کلمه کلیدی «let» (و «const») بالا می‌روند، اما بر خلاف «var»، <i>initialized</i> نمی‌شوند. آنها قبل از خطی که ما آنها را اعلام می کنیم (ابتدایی) در دسترس نیستند. این "منطقه مرده موقت" نامیده می شود. زمانی که سعی می کنیم به متغیرها قبل از اعلان دسترسی داشته باشیم، جاوا اسکریپت یک 'ReferenceError' ایجاد می کند.
