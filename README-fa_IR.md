...
درون این تابع, ما در ابتدا متغییر `name` را با کلمه‌ کلیدی `var` تعریف کرده‌ایم. این بدان معنی است که متغیر با مقدار پیش فرض `undefined` در حافظه تعریف می‌شود, until we actually get to the line where we define the variable. We haven't defined the variable yet on the line where we try to log the `name` variable, so it still holds the value of `undefined`.
