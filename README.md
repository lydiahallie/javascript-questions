...
Since we try to log `name`, a variable that is not defined, `undefined` is returned on the left side assignment. 
	
If we try the same code in a Node runtime environment, we will get `ReferenceError` because identifier `name` is not defined.
