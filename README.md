...
- A: SyntaxError while defining `getUser`, however `getList` supposed to return `[1, [2, 3, 4]]`
...
The `getUser` function receives an object. With arrow functions, we don't _have_ to write curly brackets if we just return one value. However, if you want to return an _object_ from an arrow function, you have to write it between parentheses, otherwise it throws SyntaxError error while definition! The following function would be the correct way of defining an arrow function that returns an object:
