...
- [x] `undefined` and `ReferenceError`
...
In the second loop, the variable `i` was declared using the `let` keywor[ ] variables declared with the `let` (and `const`) keyword are block-scoped (a block is anything between `{ }`). During each iteration, `i` will have a new value, and each value is scoped inside the loop.
...
- [x] `Hmm.. You don't have an age I guess`
...
- [x] When the user closes the tab.
...
- [x] `"Just give Lydia pizza already!"`
...
First, we initialize the generator function with `i` equal to `10`. We invoke the generator function using the `next()` method. The first time we invoke the generator function, `i` is equal to `10`. It encounters the first `yield` keywor[ ] it yields the value of `i`. The generator is now "paused", and `10` gets logged.
...
Then, we declare a variable `x` with the value of `y`, which is `10`. Variables declared with the `let` keyword are _block scoped_, they are only defined within the block they're declared in; the immediately invoked function expression (IIFE) in this case. When we use the `typeof` operator, the operand `x` is not define[ ] we are trying to access `x` outside of the block it's declared in. This means that `x` is not defined. Values who haven't been assigned a value or declared are of type `"undefined"`. `console.log(typeof x)` returns `"undefined"`.
...
- [x] `{ admin: true, name: "Lydia", age: 21 }`
...
- [x] `1` `2` and `undefined` `3` and `undefined` `4`
...
- [x] `running sum.js`, `running index.js`, `3`
...
- [x] `" Lydia Hallie"`, `"Lydia Hallie"` (`"[1x whitespace]Lydia Hallie"`, `"Lydia Hallie"`)
- [ ] `"Lydia Hallie"`, `"Lyd"`,
...
- [x] `game.next().value` and `game.next("Yes").value`
...
`` const path = `[ ]\Documents\Projects\table.html` ``

Which would result in:

`"[ ]DocumentsProjects able.html"`

With `String.raw`, it would simply ignore the escape and print:

`[ ]\Documents\Projects\table.html`
...
- [x] `Calculated! 20` `From cache! 20` `From cache! 20`
...
We call the `addFunction` function three times with the same value: on the first invocation, the value of the function when `num` is equal to `10` isn't cached yet. The condition of the if-statement `num in cache` returns `false`, and the else block gets execute[ ] `Calculated! 20` gets logged, and the value of the result gets added to the cache object. `cache` now looks like `{ 10: 20 }`.
...
- [x] The result of the callback in the previous `.then()`.
...
- [x] `{ default: function default(), name: "Lydia" }`
...
Then, we try to use the `.push` method on `newList`. Since `newList` is the numerical value `4`, we cannot use the `.push` metho[ ] a TypeError is thrown.
...
- [x] `a is bigger`, `undefined` and `b is bigger`, `undefined`
...
- [x] `Impossible! You should see a therapist after so much JavaScript lol`
...
- [x] `second`, `I have resolved!` and `I have resolved!`, `second`
...
- [x] Parses JSON to a JavaScript value
...
- [x] The `setInterval` callback will still be called every second
...
- [x] `Mara` `undefined` `Lydia Hallie` `ReferenceError`
...
`member.getLastName?.()`: `member` is not define[ ] `member.getLastName()` is nullish. The expression returns `undefined`.
...
- [x] We don't have to buy bananas
- [ ] `undefined`
- [ ] `1`
...
- [x] The driver drove 130 mph and has to pay \$300.00
...
- [ ] `Woah some cool data`
- [ ] `Oh finally!`
- [x] `Woah some cool data` `Oh finally!`
...
- [x] `Last line! Promise! Promise! Last line! Timeout! Timeout!`
...
- [ ] `Accessed a property!`
- [x] `Added a new property!` `Accessed a property!`
- [ ] Nothing gets logged
...
- [x] `person.address.street = "101 Main St"`
...
- [x] `yield* getMembers(teams[i].members)`
...
- [x] `*[Symbol.iterator]() { yield* Object.values(this) }`
