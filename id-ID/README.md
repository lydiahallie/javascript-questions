...
#### Jawaban: A
`String` adalah konstruktor built-in, yang dapat kita tambahkan propertinya. Saya baru saja menambahkan metode ke prototipe-nya. String primitif secara otomatis diubah menjadi objek string, yang dihasilkan oleh fungsi prototipe string. Jadi, semua string (objek string) memiliki akses ke metode itu!
...
Kita punya sebuah `setTimeout` function dan invoked diawal. Namun sekarang, Itu telah masuk akhirnya.

Karna browser ini, kami tidak hanya memiliki mesin runtime, kami juga memiliki sesuatu yang disebut `WebAPI`. `WebAPI` memberi kita fungsi` setTimeout` untuk memulai, dan misalnya DOM.

Setelah _callback_ didorong ke WebAPI, fungsi `setTimeout` itu sendiri (tetapi bukan callback!) Dimunculkan dari tumpukan.
...
###### 32. Ketika klik paragraf, apa output yang akan keluar?
...
jika kita klik `p`, kota lihat 2 keluaran yakni : `p` dan `div`. Dal propagansi, Ada 3 fase: menangkap, target, dan menggemung. By default, event handlers are executed in the bubbling phase (unless you set `useCapture` to `true`). It goes from the deepest nested element outwards.
