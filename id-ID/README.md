...
Method `Promise.all` menjalankan promies yang diberikan secara paralel. Jika satu promies gagal, method `Promise.all` dengan nilai promies yang ditolak. Dalam kasus ini, `promise3` ditolak dengan nilai` "Third" `. Kami menangkap nilai yang ditolak dalam method `catch` yang dirantai pada pemanggilan` runPromises` untuk menangkap setiap kesalahan dalam fungsi `runPromises`. Hanya `" Third "` yang dicatat, karena `promise3` ditolak dengan nilai ini.
